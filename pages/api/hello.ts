...
const {getTokenBalances: ankrGetTokenBalances} = require("./../../utils/ankr-config") 
...
export default async function handler(
  ...
  try {
    ...
    if(user) {
      // const tokenResponse = await getTokenBalances(user);
      ankrTokenResponse = await ankrGetTokenBalances(user)
    }
